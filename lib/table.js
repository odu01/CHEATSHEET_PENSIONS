...
import { formatterFor, int, num, year, isYearColumn } from './format.js';
...
export function buildTable(rows, columns, opts = {}) {
  ...
  function draw() {
    ...
    if (state.key) {
      ...
      const numeric = col && (col.type === 'number' || col.type === 'int');
    ...
    for (const r of out) {
      ...
      for (const col of columns) {
        ...
        else if (numeric) td.textContent = (col.format || formatterFor(col.unit, col.decimals))(v);
...
export function columnsFor(spec, columns, rows) {
  ...
  return keys.map(k => {
    ...
    const col = { key: k, label, type, unit,
      decimals: (k === spec.y || k === spec.value) ? spec.decimals : undefined };
