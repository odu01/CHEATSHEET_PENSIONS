...
function layoutFor(spec, t, opts) {
  ...
  const monthTicks = spec.xTickFormat === 'monthName'
    ...
    : null;
  // Na úzkej ploche sa kocka premietne širšie než plátno a štítky mesiacov sa
  // odrežú o ľavý okraj. Kamera preto o pár percent odstúpi — perspektíva
  // zostane rovnaká, scéna sa len zmestí celá.
  const eye = { x: 1.7, y: -1.7, z: 0.9, ...(spec.camera?.eye || {}) };
  const zoom = opts.width < 560 ? 1.34 : 1;
  const camera = { ...(spec.camera || {}),
    eye: { x: eye.x * zoom, y: eye.y * zoom, z: eye.z * zoom } };
...
/**
 * Lišta ovládania na úzkej ploche zavadzia viac, než pomôže: na telefóne ju
 * Plotly drží trvale zapnutú (dotyk nemá „hover") a jej päť ikon sadlo priamo
 * na farebnú stupnicu. Scéna sa aj tak otáča prstom, takže na malej šírke ide
 * lišta preč a stupnica zostane čitateľná.
 */
function configFor(opts) {
  return { ...CONFIG, displayModeBar: opts.width < 560 ? false : 'hover' };
}
...
async function surface3d(container, spec, rows, opts) {
  ...
  const trace = {
    ...
    // Stupnica ide na úzkej ploche pod graf: navrchu vpravo brala 90 z 366 px a
    // kocka sa musela stlačiť do zvyšku, takže názvy osí („Mesiac", „Rok")
    // skončili za okrajom. Naležato je z nej pás vysoký 30 px.
    colorbar: opts.width < 560
      ? { orientation: 'h', thickness: 8, len: 0.86, x: 0.5, xanchor: 'center',
          y: 0, yanchor: 'bottom', outlinewidth: 0,
          tickfont: { size: 10, color: t.inkMuted },
          title: { text: spec.unit ?? '', font: { size: 10, color: t.inkSecondary }, side: 'top' } }
  ...
  await Plotly.newPlot(container, [trace], layoutFor(spec, t, opts), configFor(opts));
  return {
...
async function scatter3d(container, spec, rows, opts) {
  ...
  await Plotly.newPlot(container, traces, layoutFor(spec, t, opts), configFor(opts));
  return {
    dispose: () => Plotly.purge(container),
