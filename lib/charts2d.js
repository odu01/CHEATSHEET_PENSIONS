...
function line(container, spec, rows, opts) {
  ...
  const [lo, hi] = extent(clean, spec.y);
...
function area(container, spec, rows, opts) {
  ...
  if (stacked) {
    ...
    for (const r of clean) totals.set(groupKey(r[spec.x]), (totals.get(groupKey(r[spec.x])) || 0) + r[spec.y]);
...
function bars(container, spec, rows, opts) {
  ...
  const vFmt = tickFormatter(spec.unit, extent(clean, spec.y));
