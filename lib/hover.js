...
function place(tip, card, px, py) {
  ...
  // V posúvateľnej ploche je počiatok absolútneho umiestnenia na začiatku
  // obsahu, nie viditeľného okna — bez scrollLeft by bublina odskočila o to,
  // ako ďaleko je graf odsunutý.
  tip.style.transform =
    `translate(${Math.round(x + card.scrollLeft)}px, ${Math.round(y + card.scrollTop)}px)`;
}
...
export function attachHover(stage, plot, hover, spec) {
  ...
  else if (hover.kind === 'flow') buildFlows();

  overlay.addEventListener('pointerleave', hide);

  /**
   * Prúdový diagram: cieľom je samotná stužka alebo uzol, nie poloha na osi.
   * Značky si nesú `data-flow` s indexom do zoznamu, takže tooltip nemusí nič
   * počítať zo škál — tie tu ani neexistujú, SVG je kreslené ručne.
   *
   * Poloha tooltipu sa berie z kurzora prepočítaného na kartu, nie z SVG
   * súradníc: SVG sa škáluje na šírku karty a pixely by nesedeli.
   */
  function buildFlows() {
    const marks = plot.querySelectorAll('[data-flow]');
    if (!marks.length) return;
    // Overlay leží nad SVG a bral by kurzor stužkám; pri tomto modeli je cieľom
    // samotná značka, tak ho treba nechať prejsť.
    overlay.classList.add('viz-overlay-through');
    const show = (el, ev) => {
      const info = hover.flows[Number(el.dataset.flow)];
      if (!info) return;
      clearActive();
      el.classList.add('viz-hit-active');
      activeEl = el;
      tip.replaceChildren(tipHead(info.head));
      for (const [value, label, color] of info.rows) tip.appendChild(tipRow(label, value, color, 'rect'));
      const box = stage.getBoundingClientRect();
      const r = el.getBoundingClientRect();
      const px = ev ? ev.clientX - box.left : r.left + r.width / 2 - box.left;
      const py = ev ? ev.clientY - box.top : r.top + r.height / 2 - box.top;
      place(tip, stage, px, py);
    };
    ...
    // Stužky sú v SVG pod overlayom, takže pointerleave overlayu tu neplatí —
    // schováva sa pri odchode z celej plochy.
